...
print("max_l", math.log2(seq_len // n_cache))
...
torch_naive_yi, torch_naive_ya = ref_naive_fwd(x, xa, W, alpha, activation, n_cache, n_head, module_l)
...
cpp_dW, W.grad = W.grad.clone(), None
...
tri_yi, tri_ya = fused_ncn_triton(x, xa, W, alpha,activation,n_head, n_cache, module_l)
...
try:
    torch.testing.assert_close(torch_naive_dW, cpp_dW, rtol=0, atol=1e-02)
    print('dW values sanity check pytorch, cuda:', torch.testing.assert_close(torch_naive_dW, cpp_dW, rtol=0, atol=1e-02))
except AssertionError as e:
    print('dW values sanity check pytorch, cuda:',e)
...
try:
    torch.testing.assert_close(torch_naive_dW, tri_dW, rtol=0, atol=1e-02)
    print('dW values sanity check pytorch, triton:', torch.testing.assert_close(torch_naive_dW, tri_dW, rtol=0, atol=1e-02))
except AssertionError as e:
    print('dW values sanity check pytorch, triton:',e)
