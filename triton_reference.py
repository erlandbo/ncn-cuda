...
def ref_tanh(x):
    # Tanh is just a scaled sigmoid
    return torch.tanh(x)  #2 * torch.sigmoid(2 * x) - 1


def torch_reference(x, xa_init, W, norm_alpha, norm_beta, norm_gamma, group_tensor, seqlen_offsets, ALPHA, ACTIVATION, GROUP, nc):
    B, L, E = x.shape
    
    yi_outputs, ya_outputs = [], []

    for i in range(0, L, GROUP):
        yi_outputs_c, ya_outputs_c = [], []
        for c in range(nc):
            feat = E // nc
            xi = x[:, i:i+GROUP, feat * c : feat * (c+1)]
            xa = xa_init[:, i:i+GROUP, feat * c : feat * (c+1)]
            Wi = W[feat * c : feat * (c+1)]
            Wj = W[E + feat * c : E + feat * (c+1)]

            for j in range(GROUP):
                
                #import pdb; pdb.set_trace()
                #x_tmp = x[:, i:i+GROUP]
                #xj = x_tmp[:, j].unsqueeze(1).broadcast_to(xi.shape)
                ...
                Wij = torch.matmul(xi, Wi) + torch.matmul(xj, Wj) 
                T = ALPHA * xi + (1.0-ALPHA) * Wij[:, None] * xj  # (B,2E)
            ...
            yi_outputs_c.append(xi)
            ya_outputs_c.append(xa)
            #return yi, ya
        
        yi_outputs.append(torch.cat(yi_outputs_c, 2))
        ya_outputs.append(torch.cat(ya_outputs_c, 2))
...
def test_op(BATCH_SIZE, SEQ_LEN, EMBED_DIM, dtype=torch.float32):
    ...
    norm_alpha = (torch.ones((2,), dtype=dtype, device="cuda").mul_(1.0).requires_grad_())
    norm_beta = (torch.zeros((2*EMBED_DIM,), dtype=dtype, device="cuda").add_(0.0).requires_grad_())
    norm_gamma = (torch.ones((2*EMBED_DIM,), dtype=dtype, device="cuda").mul_(1.0).requires_grad_())
